...
def _filter_relevant_value_sets(
    ...
    assert not (missing_value_sets), "Did not find {} value sets".format(
        missing_value_sets
    )
    ...
    dfs_relevant_value_sets["icdproc"] = df_relevant_value_sets.where(
        ...
        spark_funcs.regexp_replace(spark_funcs.col("code"), r"\.", "").alias("icdproc"),
        spark_funcs.when(
...
def _summ_claim_diag_value_sets(
    valid_dates_df: DataFrame, dfs_relevant_refs: "typing.Mapping[str, DataFrame]"
    ...
    claim_diag_value_sets = valid_diags.groupby("member_id", "claimid").agg(
        spark_funcs.collect_set("value_set_name_icddiag").alias(
            "value_set_name_icddiag"
        )
    )
...
def _summ_claim_proc_value_sets(
    ...
    claim_proc_value_sets = value_set_proc_level.groupby("member_id", "claimid").agg(
        spark_funcs.collect_set("value_set_name_icdproc").alias(
            "value_set_name_icdproc"
        )
    )
...
def _identify_claim_value_sets(
    claims_df: DataFrame, reference_df: DataFrame, performance_yearstart: datetime.date
) -> DataFrame:  # pragma: no cover
    ...
    claim_diag_value_sets = _summ_claim_diag_value_sets(
        valid_dates_df, dfs_relevant_refs
    )
    claim_proc_value_sets = _summ_claim_proc_value_sets(
        valid_dates_df, dfs_relevant_refs
    )

    value_set_claim_level = (
        valid_dates_df.join(
            spark_funcs.broadcast(dfs_relevant_refs["revcode"]),
            on="revcode",
            how="left",
        )
        .join(spark_funcs.broadcast(dfs_relevant_refs["hcpcs"]), on="hcpcs", how="left")
        .join(
    ...
    claim_value_sets = (
        value_set_claim_level.groupby("member_id", "claimid")
        .agg(
            spark_funcs.collect_set("value_set_name_revcode").alias(
                "value_set_name_revcode"
            ),
            spark_funcs.collect_set("value_set_name_hcpcs").alias(
                "value_set_name_hcpcs"
            ),
            spark_funcs.collect_set("value_set_name_billtype").alias(
                "value_set_name_billtype"
            ),
            spark_funcs.max(
                spark_funcs.when(
                    spark_funcs.col("dischargestatus") == "20", spark_funcs.lit(True)
        ...
        .join(claim_diag_value_sets, on=["member_id", "claimid"], how="left")
        .join(claim_proc_value_sets, on=["member_id", "claimid"], how="left")
    )
    ...
    udf_concat = spark_funcs.udf(
        concat_arrays, spark_types.ArrayType(spark_types.StringType())
    )
...
def _flag_calculation_steps(
    ...
    claims_calc_flags = claim_value_sets.select(
        ...
        spark_funcs.when(
            spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Inpatient Stay"
            )
            & ~spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Nonacute Inpatient Stay"
            ),
        ...
        spark_funcs.when(
            ...
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Pregnancy"
            )
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Perinatal Conditions"
            ),
            spark_funcs.lit(True),
        )
        .otherwise(spark_funcs.lit(False))
        ...
        spark_funcs.when(
            spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Chemotherapy"
            )
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Rehabilitation "
            )
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Kidney Transplant"
            )
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Bone Marrow Transplant"
            )
            | spark_funcs.array_contains(
                spark_funcs.col("value_set_names"), "Organ Transplant Other Than Kidney"
            )
            | (
                spark_funcs.array_contains(
                    spark_funcs.col("value_set_names"), "Potentially Planned Procedures"
                )
                & ~spark_funcs.array_contains(
                    spark_funcs.col("value_set_names"), "Acute Condition"
                )
            ),
            spark_funcs.lit(True),
        )
    ...
    date_reagg = (
        claims_calc_flags.where(spark_funcs.col("acute_inpatient"))
        .groupby("member_id", "admitdate", "dischdate")
        .agg(
            ...
            spark_funcs.max(spark_funcs.col("died_during_stay")).alias(
                "died_during_stay"
            ),
        )
    )
    ...
    sorted_stay_window = (
        Window().partitionBy("member_id").orderBy("admitdate", "dischdate")
    )
    flag_transfers = (
        date_reagg.select(
            "*",
            spark_funcs.lag("dischdate")
            .over(sorted_stay_window)
            .alias("last_dischdate"),
        ...
        .withColumn(
            ...
            spark_funcs.when(
                spark_funcs.datediff(
                    spark_funcs.col("admitdate"), spark_funcs.col("last_dischdate")
                )
                <= 1,
                spark_funcs.lit(True),
            ).otherwise(spark_funcs.lit(False)),
        ...
        .withColumn(
            "transfer_claimid",
            spark_funcs.when(
                spark_funcs.col("is_transfer"), spark_funcs.col("last_claimid")
            ).otherwise(spark_funcs.col("claimid")),
        )
        .withColumn(
            "planned_flag",
            spark_funcs.when(
                spark_funcs.col("is_transfer"), spark_funcs.lit(False)
            ).otherwise(spark_funcs.col("planned_flag")),
        )
    )
    transfer_reagg = flag_transfers.groupby("member_id", "transfer_claimid").agg(
    ...
    planned_flag = transfer_reagg.select(
        ...
        spark_funcs.lead("planned_flag")
        .over(sorted_stay_window)
        .alias("next_planned_flag"),
    ).withColumn(
        "exclude_planned",
        spark_funcs.when(
            (
                spark_funcs.datediff(
                    spark_funcs.col("next_admitdate"), spark_funcs.col("dischdate")
                )
                <= 30
            )
...
def _exclude_elig_gaps(denominator_events: DataFrame,) -> DataFrame:  # pragma: no cover
    ...
    summ_gaps = gaps_df.groupby("member_id", "transfer_claimid", "elig_period").agg(
        spark_funcs.sum(
            spark_funcs.when(
                spark_funcs.col("cover_medical") == "N", spark_funcs.lit(1)
            ).otherwise(spark_funcs.lit(0))
        ).alias("count_gaps"),
        spark_funcs.sum(
            spark_funcs.when(
                spark_funcs.col("cover_medical") == "N", spark_funcs.col("date_diff")
            ).otherwise(spark_funcs.lit(0))
    ...
    flag_gaps = summ_gaps.withColumn(
        ...
        spark_funcs.when(
            ...
            spark_funcs.when(
                spark_funcs.col("count_gaps") > 0, spark_funcs.lit(True)
            ).otherwise(spark_funcs.lit(False))
        ),
    )
    exclude_gap_summ = flag_gaps.groupby("member_id", "transfer_claimid").agg(
...
def _calc_elig_gap_exclusions(
    staging_calculation_steps: DataFrame, member_time: DataFrame
) -> DataFrame:  # pragma: no cover
    ...
    index_with_elig_periods = staging_calculation_steps.select(
        ...
        spark_funcs.explode(
            spark_funcs.array(
                spark_funcs.struct(
                    spark_funcs.date_sub(spark_funcs.col("dischdate"), 365).alias(
                        "elig_date_start"
                    ),
                ...
                spark_funcs.struct(
                    ...
                    spark_funcs.date_add(spark_funcs.col("dischdate"), 30).alias(
                        "elig_date_end"
                    ),
    ...
    index_stays_with_elig = (
        index_with_elig_periods.join(
            member_time.select("member_id", "cover_medical", "date_start", "date_end"),
            on="member_id",
            how="left",
        )
        .where(
        ...
        .withColumn(
            ...
            spark_funcs.greatest(
                spark_funcs.col("date_start"), spark_funcs.col("elig_date_start")
            ),
        )
        .withColumn(
            "date_end",
            spark_funcs.least(
                spark_funcs.col("date_end"), spark_funcs.col("elig_date_end")
            ),
        )
    )
    index_stay_gap_exclusion = _exclude_elig_gaps(index_stays_with_elig)
...
def _flag_readmissions(
    ...
    acute_ip_index_stays = staging_calculation_steps.join(
        index_stay_gap_exclusion, on=["member_id", "transfer_claimid"], how="left"
    ).where(
    ...
    readmissions_flagged = (
        acute_ip_index_stays.join(
            ...
            on=[
                ...
                acute_ip_numerator_stays.readmit_member_id
                == acute_ip_index_stays.member_id,
                acute_ip_numerator_stays.readmit_claimid
                != acute_ip_index_stays.transfer_claimid,
            ],
            how="left",
        )
        .withColumn(
            "has_readmit",
            spark_funcs.when(
                spark_funcs.col("readmit_claimid").isNotNull(), spark_funcs.lit(True)
            ).otherwise(spark_funcs.lit(False)),
        )
        .drop("readmit_member_id")
    )

    readmissions_dedup = readmissions_flagged.groupby(
        "member_id", "transfer_claimid", "admitdate", "dischdate"
    ).agg(spark_funcs.max("has_readmit").alias("has_readmit"))
    return readmissions_dedup


class PCR(QualityMeasure):  # pragma: no cover
    ...
    def _calc_measure(
        ...
        **kwargs
        ...
        last_eligible_dischdate = datetime.date(performance_yearstart.year, 12, 1)
        claim_value_sets = _identify_claim_value_sets(
            dfs_input["claims"], dfs_input["reference"], performance_yearstart
        )
        ...
        index_stay_gap_exclusion = _calc_elig_gap_exclusions(
            staging_calculation_steps, dfs_input["member_time"]
        )
        ...
        member_eligible_age = (
            ...
            .where((spark_funcs.col("age") >= 18) & (spark_funcs.col("age") <= 64))
        )

        results_df = (
            readmissions_dedup.join(
                member_eligible_age.select("member_id"), on="member_id", how="inner"
            )
            .groupby("member_id")
            .agg(
                spark_funcs.count("*").alias("comp_quality_denominator"),
                spark_funcs.sum(spark_funcs.col("has_readmit").cast("int")).alias(
        ...
        readmit_results = results_df.agg(
            spark_funcs.sum(spark_funcs.col("comp_quality_denominator")).alias(
                "comp_quality_denominator"
            ),
            spark_funcs.sum(spark_funcs.col("comp_quality_numerator")).alias(
                "comp_quality_numerator"
            ),
        ).collect()[0]
        LOGGER.info(
