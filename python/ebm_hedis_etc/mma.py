...
def _check_dx_array(dx_array, test_values, num_dx_codes=15):  # pragma: no cover
    test_array = [
        F.coalesce(dx_array[dx_num].isin(test_values), F.lit(False))
...
def _build_dx_array(df_columns, dx_field_name):  # pragma: no cover
    ...
    dx_column_list = [
        F.col(field) for field in df_columns if re.search(dx_field_name, field)
    ]
    ...
    if route == "oral":
        oral_events_df = (
            ...
            .agg(
                ...
                .otherwise(F.floor(F.col("dayssupply") / F.lit(30)).cast("int"))
        ...
        exploded_oral_events_df = (
            oral_events_df.withColumn(
                "disp_events", F.array([F.lit(x) for x in range(max_disp_events)])
            )
            .select(F.col("*"), F.explode("disp_events").alias("expl_disp_events"))
            .where(F.col("expl_disp_events") < F.col("num_disp_events"))
            .withColumn(
                "rx_event_number",
                F.row_number().over(
                    Window.partitionBy(
        ...
        inhaler_events_df = (
            ...
            .agg(
                F.row_number()
                .over(
                    Window.partitionBy("member_id", "fromdate", "drug_id").orderBy(
                        "member_id"
                    )
                )
        ...
        med_route_event_df = (
            ...
            .withColumn("rx_event_number", F.lit(1))
        )
...
def _initial_filtering(dfs_input, measurement_date_end):  # pragma: no cover
    ...
    diagnosis_valueset_list = [
        dx_col["diagnosis_code"] for dx_col in diagnosis_valueset_df.collect()
    ]
    ...
    med_anydiag_df = (
        dfs_input["claims"]
        .withColumn(
            "diag_array", _build_dx_array(dfs_input["claims"].columns, "icddiag")
        )
        .withColumn(
            "is_valid", _check_dx_array(F.col("diag_array"), diagnosis_valueset_list)
        )
        .drop("diag_array")
        .where(F.col("is_valid") == True)
        .join(
            F.broadcast(visit_valueset_df.where(F.col("code_system") == "UBREV")),
    ...
    med_prindiag_df = (
        ...
        .join(
            ...
            [
                ...
                dfs_input["claims"].icddiag1 == diagnosis_valueset_df.diagnosis_code,
            ],
            "inner",
        )
        .join(
    ...
    asthma_reliever_meds = ["Short-acting inhaled beta-2 agonists"]

    controller_meds_df = (
        dfs_input["ndc"]
        .where(F.col("description").isin(asthma_controller_meds))
        .withColumn("medication_type", F.lit("controller"))
    )

    reliever_meds_df = (
        dfs_input["ndc"]
        .where(F.col("description").isin(asthma_reliever_meds))
        .withColumn("medication_type", F.lit("reliever"))
    )

    rx_filtered_df = (
        dfs_input["rx_claims"]
        .join(controller_meds_df, F.col("ndc") == controller_meds_df.ndc_code, "inner")
        .union(
            dfs_input["rx_claims"].join(
                reliever_meds_df, F.col("ndc") == controller_meds_df.ndc_code, "inner"
            )
        )
    )
    ...
    for route in rx_route:
        if rx_df is None:
            rx_df = _rx_route_events(rx_filtered_df, route).select(
                F.col("member_id"),
                F.col("ndc"),
                F.col("fromdate"),
            ...
            rx_df = rx_df.union(
                _rx_route_events(rx_filtered_df, route).select(
                    F.col("member_id"),
                    F.col("ndc"),
                    F.col("fromdate"),
                    F.col("medication_type"),
    ...
    members_df = (
        dfs_input["member"]
        .where(
            F.abs(F.year(F.col("dob")) - F.year(F.lit(measurement_date_end))).between(
                5, 64
            )
        )
        .join(
            dfs_input["member_time"]
            .select(F.col("member_id"), *unique_memtime_cols)
...
def _exclusionary_filtering(
    ...
    excluded_diags_list = [
        dx_col["excluded_code"] for dx_col in excluded_diags_df.collect()
    ]
    ...
    valueset_exclusions_df = (
        ...
        .withColumn(
            "is_valid", _check_dx_array(F.col("diag_array"), excluded_diags_list)
        )
        .drop("diag_array")
        .where(
            (F.col("is_valid") == True)
    ...
    controller_exclusions_df = (
        ...
        .groupBy("member_id")
        .agg(F.collect_set("medication_type").alias("med_types"))
        .where(~F.array_contains(F.col("med_types"), "controller"))
        .select("member_id")
    )

    exclusions_df = (
        gap_exclusions_df.union(valueset_exclusions_df)
        .union(controller_exclusions_df)
        .distinct()
    )

    members_df = filtered_data_dict["members"].join(
        exclusions_df, "member_id", "left_anti"
    )
    ...
    rx_df = filtered_data_dict["rx"].join(exclusions_df, "member_id", "left_anti")
...
def _event_filtering(
    exc_filtered_data_dict, performance_yearstart, measurement_date_end
):  # pragma: no cover
    """helper function that filters claims by medical and pharmacutical
       dispensing events of interest"""
    rx_event_mask_df = (
        exc_filtered_data_dict["rx"]
        .groupBy(["member_id", "ndc", "fromdate", "medication_type"])
        .count()
    )

    med_eventmask_any_df = (
        exc_filtered_data_dict["med_anydiag"]
        .groupBy(["member_id", "visit_valueset_name", "fromdate"])
        .agg(F.countDistinct(F.col("visit_valueset_name")).alias("count"))
    )

    med_eventmask_prin_df = (
        exc_filtered_data_dict["med_prindiag"]
        .groupBy(
            ["member_id", "visit_valueset_name", "diagnosis_valueset_name", "fromdate"]
        )
    ...
    ).select(F.col("member_id"), F.col("is_elig"))
    ...
    out_obs_event_med_df = (
        med_eventmask_any_df.where(
            (
                F.col("visit_valueset_name").rlike(r"Outpatient")
                | F.col("visit_valueset_name").rlike(r"Observation")
            )
        )
        .groupBy("member_id", "fromdate")
        .agg(F.max("count").alias("obs_out_visit"))
        .groupBy("member_id")
        .agg(F.sum("obs_out_visit").alias("unique_service_dates"))
        .withColumn(
            "is_elig", F.when(F.col("unique_service_dates") >= 4, True).otherwise(False)
        )
        .select(F.col("member_id"), F.col("is_elig"))
    )
    ...
    out_obs_event_rx_df = (
        rx_event_mask_df.groupBy("member_id")
        .agg(F.count("*").alias("unique_disp_event"))
        .withColumn(
            "is_elig", F.when(F.col("unique_disp_event") >= 2, True).otherwise(False)
        )
        .select(F.col("member_id"), F.col("is_elig"))
    )
    ...
    asthma_disp_event_df = (
        ...
        .agg(F.count("*").alias("unique_disp_event"))
        .withColumn(
            "is_elig", F.when(F.col("unique_disp_event") >= 4, True).otherwise(False)
        )
        .select(F.col("member_id"), F.col("is_elig"))
    )
    ...
    included_year1_df = (
        exc_filtered_data_dict["rx"]
        .where(F.year(F.col("fromdate")) == measurement_date_end.year - 1)
        .groupBy("member_id")
        .agg(F.collect_set(F.col("description")).alias("rx_types"))
        .withColumn(
            "is_included",
            F.when(
                (
        ...
        .where(F.col("is_included"))
        .join(exc_filtered_data_dict["rx"], "member_id", "left_outer")
        .groupBy("member_id")
        .count()
        .where(F.col("count") >= 4)
        .select(F.col("member_id"))
    ...
    included_year2_df = (
        exc_filtered_data_dict["rx"]
        .where(F.year(F.col("fromdate")) == performance_yearstart.year)
        .groupBy("member_id")
        .agg(F.collect_set(F.col("description")).alias("rx_types"))
        .withColumn(
            "is_included",
    ...
    included_df = included_year1_df.union(included_year2_df).distinct()

    included_diag_df = included_df.join(
        med_eventmask_any_df.select(F.col("member_id")), "member_id", "inner"
    ).withColumn("is_elig", F.lit(True))

    out_obs_event_df = out_obs_event_med_df.join(
        out_obs_event_rx_df, ["member_id", "is_elig"], "inner"
    )

    elig_members_df = (
        ed_event_df.union(acute_inp_event_df)
        .union(out_obs_event_df)
        .union(asthma_disp_event_df)
        .union(included_diag_df)
        .where(F.col("is_elig") == True)
        .select(F.col("member_id"))
        .distinct()
    )

    members_df = exc_filtered_data_dict["members"].join(
        elig_members_df, "member_id", "left_semi"
    )

    med_df = exc_filtered_data_dict["med_anydiag"].join(
        members_df, "member_id", "left_semi"
    )

    rx_df = exc_filtered_data_dict["rx"].join(members_df, "member_id", "left_semi")

    return {"med": med_df, "rx": rx_df, "members": members_df}


def _calculate_rates(
    rx_data, performance_yearstart, measurement_date_end
):  # pragma: no cover
    controller_claims_df = (
        rx_data.where(
            (F.col("medication_type") == "controller")
            & F.col("fromdate").between(
                F.lit(datetime.date(measurement_date_end.year - 1, 12, 31)),
                F.lit(measurement_date_end),
            )
        )
        .select(
            ...
            F.col("drug_id"),
            F.col("dayssupply"),
            F.expr("date_add(fromdate, dayssupply - 1)").alias(
                "fromdate_to_dayssupply"
            ),
        )
        .where(F.col("dayssupply") > 0)
        .select(
            "*",
            F.create_map(
                F.lit("date_start"),
    ...
    def adjust_date_windows(
        ...
        sorted_array = sorted(
            array_coverage_windows, key=lambda x: (x["date_start"], x["date_end"])
        )
        output_array = []
        last_date_end = None
        for window_coverage in sorted_array:
            ...
            output_array.append(
                {"date_start": adj_date_start, "date_end": adj_date_end}
            )
        return output_array

    udf_adjust_date_windows = F.udf(
        adjust_date_windows, T.ArrayType(T.MapType(T.StringType(), T.DateType()))
    )

    adjusted_date_windows = collect_date_windows.select(
        "*",
        F.explode(udf_adjust_date_windows(F.col("array_coverage_windows"))).alias(
            "map_adj_coverage_windows"
        ),
    ).select(
        "*",
    ...
    decouple_overlapping_coverage = (
        decouple_common_windows(
            adjusted_date_windows, "member_id", "date_start", "date_end"
        )
        .select(
            "member_id",
            "date_start",
        ...
        .withColumn(
            "days_covered", F.datediff(F.col("date_end"), F.col("date_start")) + 1
        )
    )

    member_coverage_summ = (
        decouple_overlapping_coverage.groupby("member_id")
        .agg(
        ...
        .select(
            "*",
            F.datediff(
                F.lit(datetime.date(performance_yearstart.year, 12, 31)), F.col("ipsd")
            ).alias("treatment_period"),
        )
        .withColumn(
            "days_covered", F.least(F.col("days_covered"), F.col("treatment_period"))
        )
        .withColumn(
...
def calculate_numerator(
    ...
    rx_data = rx_data.join(
        dfs_input["rx_claims"], ["member_id", "ndc", "fromdate", "dayssupply"], "inner"
    )
...
class MMA(QualityMeasure):
    ...
    def _calc_measure(
        ...
        measurement_date_end = datetime.date(performance_yearstart.year, 12, 31)
        ...
        denom_final_df = (
            ...
            .select(F.col("member_id"))
            .distinct()
            .join(denom_df["rx"].select(F.col("member_id")), "member_id", "inner")
            .join(denom_df["med"].select(F.col("member_id")), "member_id", "inner")
            .distinct()
            .select(
                F.col("member_id"),
                F.when(F.col("member_id").isNotNull(), F.lit(1))
                .otherwise(F.lit(0))
        ...
        result_df = denom_final_df.join(numer_final_df, "member_id", "full").select(
            F.col("member_id"),
            F.coalesce(F.col("comp_quality_numerator"), F.lit(0)).alias(
                "comp_quality_numerator"
            ),
            F.coalesce(F.col("comp_quality_denominator"), F.lit(0)).alias(
