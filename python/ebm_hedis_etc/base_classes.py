...
class QualityMeasure(metaclass=abc.ABCMeta):
    ...
    def _validate_schema(self, df_: DataFrame) -> None:  # pragma: no cover
        ...
        for field in self.key_fields:
            assert field in df_.columns, "Result does not contain key field {}".format(
                field
            )
        for name in df_.columns:
            if name in self.key_fields:
                continue
            assert self.validate_meausure_column_name(
                name
    ...
    def calc_measure(
        ...
        **kwargs
        ...
        measure_results = self._calc_measure(dfs_input, performance_yearstart, **kwargs)
