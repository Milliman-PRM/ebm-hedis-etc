...
def _calc_simple_cis_measure(
    ...
    claims_df = eligible_claims_df.join(
        reference_values, eligible_claims_df.hcpcs == reference_values.code, "inner"
    ).where(
        spark_funcs.datediff(spark_funcs.col("fromdate"), spark_funcs.col("dob"))
        >= days_between_dob
    )

    vaccines_df = claims_df.groupBy("member_id").agg(
        spark_funcs.countDistinct("fromdate").alias("vaccine_count")
    )

    output_df = (
        member_df.select(spark_funcs.col("member_id").alias("base_member_id"))
        .join(
            eligible_members_df,
            spark_funcs.col("base_member_id") == spark_funcs.col("member_id"),
            how="left_outer",
        )
        .join(vaccines_df, "member_id", how="left_outer")
        .select(
            spark_funcs.col("base_member_id").alias("member_id"),
            spark_funcs.when(
                spark_funcs.col("vaccine_count") >= vaccine_count, spark_funcs.lit(1)
            )
            .otherwise(spark_funcs.lit(0))
            .alias("comp_quality_numerator"),
            spark_funcs.when(
                eligible_members_df.member_id.isNotNull(), spark_funcs.lit(1)
            )
            .otherwise(spark_funcs.lit(0))
...
def calc_dtap(
    ...
    return (
        ...
        .drop("vaccine_count")
        .withColumn("comp_quality_short", spark_funcs.lit("CIS - DTaP"))
    )
...
def calc_ipv(
    ...
    return (
        ...
        .drop("vaccine_count")
        .withColumn("comp_quality_short", spark_funcs.lit("CIS - IPV"))
    )
...
def calc_hib(
    ...
    return (
        ...
        .drop("vaccine_count")
        .withColumn("comp_quality_short", spark_funcs.lit("CIS - HiB"))
    )
...
def calc_pneumococcal(
    ...
    return (
        ...
        .drop("vaccine_count")
        .withColumn(
            "comp_quality_short", spark_funcs.lit("CIS - Pneumococcal Conjugate")
        )
    )
...
def calc_influenza(
    ...
    return (
        ...
        .drop("vaccine_count")
        .withColumn("comp_quality_short", spark_funcs.lit("CIS - Influenza"))
    )
...
def calc_hepatitis_b(
    ...
    diags_explode_df = eligible_claims_df.select(
        ...
        spark_funcs.explode(
            spark_funcs.array(
                [
                    spark_funcs.col(col)
                    for col in eligible_claims_df.columns
    ...
    history_hepatitis_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Hepatitis B"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
                == spark_funcs.regexp_replace(reference_df.code, r"\.", ""),
            ],
            how="inner",
        )
        .select("member_id")
    ...
    hep_b_vaccine_df = _calc_simple_cis_measure(
        ...
        0,
        3,
    ).select("member_id", "vaccine_count")
    ...
    newborn_hepatitis_df = (
        procs_explode_df.join(
            ...
            [
                procs_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                procs_explode_df.proc
                == spark_funcs.regexp_replace(reference_df.code, r"\.", ""),
            ],
            how="left_outer",
        )
    ...
    combine_df = (
        hep_b_vaccine_df.join(newborn_hepatitis_df, "member_id", how="full_outer")
        .withColumn(
            "vaccine_count",
            spark_funcs.when(
        ...
        .where(spark_funcs.col("vaccine_count") >= 3)
    )
    ...
    output_df = (
        ...
        .select(
            ...
            spark_funcs.when(
                combine_df.member_id.isNotNull()
...
def calc_hepatitis_a(
    ...
    hep_a_vaccine_df = (
        ...
        .where(spark_funcs.col("comp_quality_numerator") == 1)
        .select("member_id", "vaccine_count")
    )
    ...
    hep_a_history_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Hepatitis A"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
    ...
    output_df = (
        ...
        .select(
            ...
            spark_funcs.when(
                hep_a_vaccine_df.member_id.isNotNull()
                | hep_a_history_df.member_id.isNotNull(),
                spark_funcs.lit(1),
            )
            .otherwise(spark_funcs.lit(0))
            .alias("comp_quality_numerator"),
...
def calc_vzv(
    ...
    vzv_history_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Varicella Zoster"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
    ...
    output_df = (
        ...
        .select(
            ...
            spark_funcs.when(
                (vzv_vaccine_df.member_id.isNotNull())
                | (vzv_history_df.member_id.isNotNull()),
                spark_funcs.lit(1),
            )
            .otherwise(spark_funcs.lit(0))
            .alias("comp_quality_numerator"),
...
def calc_rotavirus(
    ...
    two_claims_df = eligible_claims_df.join(
        ...
        spark_funcs.datediff(spark_funcs.col("fromdate"), spark_funcs.col("dob")) >= 42
    )
    ...
    two_three_combination_df = (
        two_claims_df.union(three_claims_df)
        .groupBy("member_id", "value_set_name")
        .agg(spark_funcs.countDistinct("fromdate").alias("vaccine_count"))
        .where(
            (
        ...
        .groupBy("member_id")
        .agg(spark_funcs.countDistinct("value_set_name").alias("vaccine_type_count"))
        .where(spark_funcs.col("vaccine_type_count") > 1)
    )
...
def calc_mmr(
    ...
    mumps_history_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Mumps"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
    ...
    mumps_vaccine_or_history_df = mumps_history_df.union(
        mumps_vaccine_df.select("member_id")
    ).distinct()

    measles_rubella_and_mumps_df = (
        measles_rubella_vaccine_df.select("member_id")
        .intersect(mumps_vaccine_or_history_df)
        .distinct()
    )
    ...
    measles_history_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Measles"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
    ...
    measles_vaccine_or_history_df = (
        measles_vaccine_df.select("member_id").union(measles_history_df).distinct()
    )
    ...
    rubella_history_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Rubella"),
            [
                diags_explode_df.icdversion
                == spark_funcs.regexp_extract(reference_df.code_system, r"\d+", 0),
                diags_explode_df.diag
    ...
    rubella_vaccine_or_history_df = (
        rubella_vaccine_df.select("member_id").union(rubella_history_df).distinct()
    )

    measles_and_rubella_and_mumps_df = measles_vaccine_or_history_df.intersect(
        mumps_vaccine_or_history_df
    ).intersect(rubella_vaccine_or_history_df)
    ...
    output_df = (
        ...
        .join(
            mmr_vaccine_df.withColumnRenamed("member_id", "mmr_vaccine_member_id"),
            spark_funcs.col("member_id") == spark_funcs.col("mmr_vaccine_member_id"),
            how="left_outer",
        )
        .join(
            measles_rubella_and_mumps_df.withColumnRenamed(
                "member_id", "meas_rub_and_mumps_member_id"
            ),
            spark_funcs.col("member_id")
            == spark_funcs.col("meas_rub_and_mumps_member_id"),
            how="left_outer",
        )
        .join(
            measles_and_rubella_and_mumps_df.withColumnRenamed(
                "member_id", "meas_and_rub_and_mumps_member_id"
            ),
            spark_funcs.col("member_id")
            == spark_funcs.col("meas_and_rub_and_mumps_member_id"),
            how="left_outer",
        )
        .select(
            spark_funcs.col("base_member_id").alias("member_id"),
...
class CIS(QualityMeasure):  # pragma: no cover
    ...
    def _calc_measure(
        ...
        elig_pop_covered = (
            ...
            .where(spark_funcs.col("cover_medical").isin("Y"))
            .join(
                dfs_input["member"].select("member_id", "dob"),
                "member_id",
                "left_outer",
            )
            .withColumn(
                "second_birthday", spark_funcs.date_add(spark_funcs.col("dob"), 365 * 2)
            )
            .where(
                spark_funcs.col("second_birthday").between(
                    spark_funcs.lit(measure_start), spark_funcs.lit(measure_end)
                )
            )
        )
        ...
        gaps_df = (
            decoupled_windows.join(
                elig_pop_covered, ["member_id", "date_start", "date_end"], "left_outer"
            )
        ...
        long_gap_df = gaps_df.where(
            spark_funcs.col("date_start").between(
                spark_funcs.date_sub(spark_funcs.col("dob"), 365),
                spark_funcs.col("dob"),
            )
        ...
        gap_count_df = (
            ...
            .where(spark_funcs.col("num_of_gaps") > 1)
            .select("member_id")
        )

        excluded_members_df = long_gap_df.union(gap_count_df).distinct()

        eligible_members_df = (
            elig_pop_covered.select("member_id", "second_birthday")
            .distinct()
            .join(excluded_members_df, "member_id", how="left_outer")
            .where(excluded_members_df.member_id.isNotNull())
        )

        eligible_claims_df = (
            dfs_input["claims"]
            .join(eligible_members_df, "member_id", how="inner")
            .where(spark_funcs.col("fromdate") <= spark_funcs.col("second_birthday"))
        )
