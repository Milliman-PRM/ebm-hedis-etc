...
class PBH(QualityMeasure):
    ...
    def _calc_measure(
        ...
        inpatient_disch_ref_df = (
            reference_df.where(
                spark_funcs.col("value_set_name").isin(
                    "Inpatient Stay", "Nonacute Inpatient Stay"
                )
            )
            .groupBy("code")
            .count()
            .where(spark_funcs.col("count") < 2)
            .join(
                reference_df.where(
        ...
        acute_inpat_disch_df = (
            ...
            .withColumn(
                "transfer_date", spark_funcs.date_sub(spark_funcs.col("admitdate"), 1)
            )
        )
        ...
        non_acute_inpat_disch_df = (
            ...
            .select(
                "claimid",
                "member_id",
                spark_funcs.date_sub(spark_funcs.col("admitdate"), 1).alias(
                    "transfer_date"
                ),
            )
        )
        ...
        diags_explode_df = acute_inpat_disch_df.select(
            ...
            spark_funcs.explode(
                spark_funcs.array(
                    [
                        spark_funcs.col(col)
                        for col in dfs_input["claims"].columns
        ...
        ami_diag_df = (
            diags_explode_df.join(
                reference_df.where(spark_funcs.col("value_set_name") == "AMI"),
                [
            ...
            .select("claimid", "member_id", "dischdate")
            .distinct()
        )
        ...
        direct_transfer_df = (
            ...
            .join(
                acute_inpat_disch_df.withColumnRenamed(
                    "member_id", "join_member_id"
                ).withColumnRenamed("dischdate", "transfer_dischdate"),
                [
            ...
            .where(non_acute_inpat_disch_df.transfer_date.isNull())
            .select(
                ami_diag_df.claimid,
                ami_diag_df.member_id,
                spark_funcs.coalesce(
                    spark_funcs.col("transfer_dischdate"), spark_funcs.col("dischdate")
                ).alias("dischdate"),
            )
        )

        multiple_episode_window = (
            Window().partitionBy("member_id").orderBy("dischdate", "claimid")
        )

        ami_reduce_df = (
            direct_transfer_df.withColumn(
                "row", spark_funcs.row_number().over(multiple_episode_window)
            )
            .where(spark_funcs.col("row") == 1)
            .drop("row")
        )
        ...
        elig_pop_covered = (
            ...
            .join(ami_reduce_df, "member_id", how="inner")
        )
        ...
        gaps_df = (
            ...
            .join(
                ami_reduce_df.select("member_id", "dischdate"),
                "member_id",
                how="left_outer",
            )
        )
        ...
        long_gap_df = gaps_df.where(
            spark_funcs.col("date_start").between(
                ...
                spark_funcs.date_add(spark_funcs.col("dischdate"), 179),
            )
        ...
        excluded_members_df = (
            long_gap_df.union(gap_count_df)
            .distinct()
            .withColumnRenamed("member_id", "exclude_member_id")
        )

        eligible_members_df = (
            elig_pop_covered.select("member_id", "dischdate")
            .distinct()
            .join(
                excluded_members_df,
        ...
        eligible_rx_claims_df = (
            ...
            .select(
                ...
                "claimid",
                "fromdate",
                spark_funcs.expr("date_add(fromdate, dayssupply)").alias(
                    "todate_dayssupply"
                ),
                "dayssupply",
                "dischdate",
            ...
            .withColumn(
                ...
                spark_funcs.greatest(
                    spark_funcs.col("dischdate"), spark_funcs.col("fromdate")
                ),
            )
            .withColumn(
                "coverage_end",
        ...
        coverage_count_df = decoupled_coverage_df.groupBy("member_id").agg(
            spark_funcs.sum(
                spark_funcs.datediff(
                    spark_funcs.col("coverage_end"), spark_funcs.col("coverage_start")
                )
            ).alias("beta_covered_days")
        )

        numer_df = coverage_count_df.where(spark_funcs.col("beta_covered_days") >= 135)
