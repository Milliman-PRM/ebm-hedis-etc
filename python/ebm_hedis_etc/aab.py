...
def _exclude_elig_gaps(denominator_events: DataFrame,) -> DataFrame:  # pragma: no cover
    ...
    cover_medical_windows = denominator_events.select(
        "member_id", "fromdate", "date_start", "date_end", "cover_medical"
    )
    ...
    summ_gaps = gaps_df.groupby("member_id", "fromdate").agg(
        spark_funcs.sum(
            spark_funcs.when(
...
def _calculate_age_criteria(
    ...
    elig_members = members.select(
        ...
        (
            spark_funcs.datediff(
                spark_funcs.lit(date_performanceyearstart_prior), spark_funcs.col("dob")
            )
            / 365.25
        ).alias("age_py_start_prior"),
        (
            spark_funcs.datediff(
                spark_funcs.lit(date_performanceyearend), spark_funcs.col("dob")
            )
            / 365.25
        ).alias("age_py_end"),
    ).select(
...
def _prep_reference_data(
    value_set_reference_df: DataFrame, ndc_reference_df: DataFrame
    ...
    map_med_reference_codesets = {
        "index_visit": ["Outpatient", "Observation", "ED"],
        "index_diag": ["Acute Bronchitis"],
        "index_ip_excl": ["Inpatient Stay"],
        "negative_conditions": [
            "HIV",
            "HIV Type 2",
            "Malignant Neoplasms",
        ...
        "negative_comp_diag": ["Pharyngitis", "Competing Diagnosis"],
    }
    ...
    value_set_munge = value_set_reference_df.withColumn(
        "code", spark_funcs.regexp_replace(spark_funcs.col("code"), r"\.", "")
    ).withColumn(
        "icdversion",
        spark_funcs.when(
            spark_funcs.col("code_system").contains("ICD"),
            spark_funcs.regexp_extract(spark_funcs.col("code_system"), r"\d+", 0),
        ),
    )
    ...
    for key_reference, iter_code_sets in map_med_reference_codesets.items():
        ...
        found_value_sets = {
            row["value_set_name"]
            for row in map_references[key_reference]
        ...
        assert not (
            missing_value_sets
        ), "Did not find {} value sets in {} grouping".format(
            missing_value_sets, key_reference
        )
...
def _identify_aab_prescriptions(
    outpharmacy: DataFrame, map_references: "typing.Mapping[str, DataFrame]"
) -> DataFrame:  # pragma: no cover
    ...
    aab_rx = (
        outpharmacy.join(
            spark_funcs.broadcast(
                map_references["aab_rx"].select(
                    spark_funcs.col("ndc_code").alias("ndc")
                )
            ),
            on="ndc",
            how="inner",
        ...
        .agg(spark_funcs.max("dayssupply").alias("dayssupply"))
    )
...
def _identify_acute_bronchitis_events(
    ...
    index_cpt_hcpcs = (
        map_references["index_visit"]
        .filter(spark_funcs.col("code_system").isin("HCPCS", "CPT"))
        .select(
    ...
    index_rev = (
        map_references["index_visit"]
        .filter(spark_funcs.col("code_system") == "UBREV")
        .select(
            spark_funcs.col("code").alias("revcode"),
            spark_funcs.lit(1).alias("index_rev"),
        )
    )
    assert (index_cpt_hcpcs.count() + index_rev.count()) == map_references[
        "index_visit"
    ].count(), 'Some "index_visit" code systems have not been accounted for'

    assert (
        map_references["index_diag"].select("code_system").distinct().count() == 1
    ), 'Some "index_diag" code systems have not been accounted for'
    assert (
        map_references["index_ip_excl"].select("code_system").distinct().count() == 1
    ), 'Some "index_ip_excl" code systems have not been accounted for'

    index_events = (
        outclaims.join(spark_funcs.broadcast(index_cpt_hcpcs), on="hcpcs", how="left")
        .join(spark_funcs.broadcast(index_rev), on="revcode", how="left")
        .filter(
            (spark_funcs.col("index_cpt") == 1) | (spark_funcs.col("index_rev") == 1)
        )
        .select(
            "*",
            spark_funcs.explode(
    ...
    ip_excl_dates = (
        ...
        .select("member_id", "admitdate", "ip_excl")
        .distinct()
        .alias("ip_excl")
    )
    ...
    acute_bronchitis_events = (
        index_events.join(
            ip_excl_dates,
            on=[
                spark_funcs.col("index.member_id")
                == spark_funcs.col("ip_excl.member_id"),
        ...
        .filter(spark_funcs.col("ip_excl").isNull())
        .select("index.member_id", "index.fromdate")
        .distinct()
        .filter(
...
def _identify_negative_condition_events(
    outclaims: DataFrame, map_references: "typing.Mapping[str, DataFrame]"
):  # pragma: no cover
    """Find dates for all medical events that could disqualify an index episode"""
    assert (
        map_references["negative_conditions"]
        .filter(~spark_funcs.col("code_system").isin("ICD10CM", "ICD9CM"))
        .count()
        == 0
    ...
    negative_condition_events = (
        ...
        .select("member_id", "fromdate", "negative_conditions")
        .distinct()
    )
...
def _identify_competing_diagnoses(
    ...
    assert (
        map_references["negative_comp_diag"]
        .filter(~spark_funcs.col("code_system").isin("ICD10CM", "ICD9CM"))
        .count()
        == 0
    ), 'Some "negative_conditions" code systems have not been accounted for'
    ...
    negative_comp_diags = (
        ...
        .select("member_id", "fromdate", "negative_comp_diag")
        .distinct()
    )
...
def _exclude_negative_history(
    ...
    negative_rx_scripts = aab_rx.select(
        "member_id", "fromdate", spark_funcs.lit(1).alias("negative_rx_script")
    )
    ...
    excluded_negative_conds = (
        acute_bronchitis_events.join(
            negative_condition_events,
            on=[
                acute_bronchitis_events.member_id
                == negative_condition_events.member_id,
    ...
    excluded_negative_rx = (
        excluded_negative_conds.join(
            negative_rx_scripts,
            on=[
    ...
    excluded_negative_rx_active = (
        excluded_negative_rx.join(
            ...
            on=[
                ...
                excluded_negative_rx.fromdate.between(
                    negative_rx_active_supply.fromdate, negative_rx_active_supply.todate
                ),
    ...
    excluded_negative_history = (
        excluded_negative_rx_active.join(
            negative_comp_diags,
            on=[
...
def _apply_elig_gap_exclusions(
    ...
    summ_gaps = _exclude_elig_gaps(index_stays_with_elig)
    elig_gap_exclusions = excluded_negative_history.join(
        summ_gaps, on=["member_id", "fromdate"]
    ).withColumn(
        "elig_exclusion",
        spark_funcs.when(
...
def _apply_age_exclusions(
    elig_gap_exclusions: DataFrame, elig_members: DataFrame
) -> DataFrame:  # pragma: no cover
    """Apply exclusions for index episodes that don't meet continuous enrollment criteria"""

    age_exclusions = elig_gap_exclusions.join(
        elig_members.select("member_id", "meets_age_criteria"), on="member_id"
    )
...
def _select_earliest_episode(
    ...
    earliest_window = (
        ...
        .orderBy(spark_funcs.desc("denominator"), "fromdate")
    )
    earliest_episode = index_eligible_events.withColumn(
        "row_order", spark_funcs.row_number().over(earliest_window)
    ).filter(spark_funcs.col("row_order") == 1)

    return earliest_episode


def _calculate_numerator(
    earliest_episode: DataFrame, aab_rx: DataFrame
):  # pragma: no cover
    """Apply AAB prescriptions to index episodes to determine numerator compliance"""

    numerator_flagged = (
        earliest_episode.join(
            aab_rx.select("member_id", "fromdate"),
            on=[
        ...
        .withColumn(
            "numerator",
            spark_funcs.when(
                (spark_funcs.col("denominator") == 1) & aab_rx.fromdate.isNotNull(),
                spark_funcs.lit(1),
            ).otherwise(spark_funcs.lit(0)),
        )
        .drop(aab_rx.member_id)
        .drop(aab_rx.fromdate)
        .distinct()
    )
...
def _format_measure_results(
    ...
    measure_formatted = numerator_flagged.select(
        ...
        spark_funcs.when(
            ...
            ),  # Inverse measure (antibiotic scripts are bad)
            spark_funcs.lit(0),
        )
        .when(spark_funcs.col("denominator") == 1, spark_funcs.lit(1))
        .otherwise(spark_funcs.lit(0))
...
def _flag_denominator(
    ...
    elig_members = _calculate_age_criteria(
        dfs_input["member"], date_performanceyearstart, date_performanceyearend
    )
    ...
    negative_condition_events = _identify_negative_condition_events(
        dfs_input["outclaims"], map_references
    )
    negative_comp_diags = _identify_competing_diagnoses(
        dfs_input["outclaims"], map_references
    )
    excluded_negative_history = _exclude_negative_history(
        acute_bronchitis_events, negative_condition_events, aab_rx, negative_comp_diags
    )
    elig_gap_exclusions = _apply_elig_gap_exclusions(
        excluded_negative_history, dfs_input["member_time"]
    )
    age_exclusions = _apply_age_exclusions(elig_gap_exclusions, elig_members)
    index_eligible_events = _flag_index_eligible_events(age_exclusions)
    earliest_episode = _select_earliest_episode(index_eligible_events)
    return earliest_episode
...
class AAB(QualityMeasure):  # pragma: no cover
    ...
    def _calc_measure(
        ...
        **kwargs
        ...
        map_references = _prep_reference_data(dfs_input["reference"], dfs_input["ndc"])
        aab_rx = _identify_aab_prescriptions(dfs_input["outpharmacy"], map_references)
        ...
        numerator_flagged = _calculate_numerator(denominator, aab_rx)
        measure_results = _format_measure_results(numerator_flagged)
