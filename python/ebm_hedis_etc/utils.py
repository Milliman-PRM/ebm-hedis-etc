...
def find_elig_gaps(
    member_time: DataFrame, ce_start_date: Column, ce_end_date: Column
    ...
    return gaps_df.groupBy("member_id").agg(
        spark_funcs.max(spark_funcs.col("elig_gap_date_diff")).alias("largest_gap"),
        spark_funcs.sum(spark_funcs.col("elig_gap_flag")).alias("gap_count"),
    )
