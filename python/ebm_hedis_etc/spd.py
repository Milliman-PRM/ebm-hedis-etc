...
def _identify_med_claims(
    ...
    restricted_claims_df = claims_df.join(
        members_no_gaps, "member_id", how="inner"
    ).where(
        (
        ...
        restricted_claims_df.join(
            reference_df.where(
                spark_funcs.col("value_set_name").isin(
                    "Outpatient", "Observation", "ED", "Nonacute Inpatient"
                )
    ...
    diags_explode_df = visits_df.select(
        ...
        spark_funcs.explode(
            spark_funcs.array(
                [
                    spark_funcs.col(col)
                    for col in restricted_claims_df.columns
    ...
    diabetes_diag_member_df = (
        diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Diabetes"),
            [
        ...
        .groupBy("member_id")
        .agg(spark_funcs.countDistinct("fromdate", "todate").alias("visit_count"))
        .where(spark_funcs.col("visit_count") >= 2)
        .select("member_id")
        .distinct()
    )

    acute_inpatient_encounter_df = (
        restricted_claims_df.join(
            reference_df.where(
    ...
    acute_inpatient_diabetes_diag_member_df = (
        acute_inpatient_diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "Diabetes"),
            [
...
def _identify_rx_claims(
    ...
    restricted_claims_df = rx_claims_df.join(
        ...
        spark_funcs.year("fromdate").isin(
            performance_yearstart.year - 1, performance_yearstart.year
        )
    )
...
def _identify_events_exclusion(
    ...
    mi_event_member_df = (
        inpatient_stays_diag_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "MI"),
            [
    ...
    cabg_event_member_df = (
        ...
        .union(
            procs_explode_df.join(
                reference_df.where(spark_funcs.col("value_set_name") == "CABG"),
                [
    ...
    pci_event_member_df = (
        ...
        .union(
            procs_explode_df.join(
                reference_df.where(spark_funcs.col("value_set_name") == "PCI"),
                [
                    procs_explode_df.icdversion == reference_df.icdversion,
                    procs_explode_df.proc == reference_df.code,
                ],
    ...
    return (
        mi_event_member_df.union(cabg_event_member_df)
        .union(pci_event_member_df)
        .union(other_revascularization_event_member_df)
        .select("member_id")
        .distinct()
    )
...
def _identify_diagnosis_exclusion(
    ...
    outpatient_ivd_diag_member_df = (
        outpatient_diags_explode_df.join(
            reference_df.where(spark_funcs.col("value_set_name") == "IVD"),
            [
...
def _measure_exclusion(
    ...
    pregnancy_member_excl_df = (
        ...
        .join(
            reference_df.where(spark_funcs.col("value_set_name") == "Pregnancy"),
            [
    ...
    ivf_member_excl_df = (
        ...
        .join(
            reference_df.where(spark_funcs.col("value_set_name") == "IVF"),
            spark_funcs.col("hcpcs") == spark_funcs.col("code"),
            how="inner",
        )
        .select("member_id")
    ...
    cirrhosis_member_excl_df = (
        ...
        .join(
            reference_df.where(spark_funcs.col("value_set_name") == "Cirrhosis"),
            [
                diag_explode_df.icdversion == reference_df.icdversion,
                diag_explode_df.diag == reference_df.code,
            ],
    ...
    cardio_disease_excl_df = (
        _identify_diagnosis_exclusion(claims_df, reference_df, performance_yearstart)
        .intersect(
        ...
        .union(
            _identify_events_exclusion(claims_df, reference_df, performance_yearstart)
        )
        .distinct()
    )

    return (
        cardio_disease_excl_df.union(pregnancy_member_excl_df)
        .union(ivf_member_excl_df)
        .union(esrd_member_excl_df)
        .union(cirrhosis_member_excl_df)
        .union(muscular_pain_member_excl_df)
        .union(estrogen_agonists_member_excl_df)
        .distinct()
    )
...
def _calc_rate_one(
    ...
    return (
        rx_claims_df.join(eligible_members_df, "member_id", how="inner")
        .where(spark_funcs.year("fromdate") == performance_yearstart.year)
        .join(
            rx_reference_df.where(
...
def _calc_rate_two(
    ...
    statin_claims_df = (
        rx_claims_df.join(eligible_members_df, "member_id", how="inner")
        .where(spark_funcs.year("fromdate").isin(performance_yearstart.year))
        .join(
        ...
        .select(
            ...
            "drug_id",
            "dayssupply",
            spark_funcs.expr("date_add(fromdate, dayssupply - 1)").alias(
    ...
    collect_date_windows = statin_claims_df.groupby("member_id", "drug_id").agg(
        spark_funcs.collect_list(spark_funcs.col("map_coverage_window")).alias(
    ...
    udf_adjust_date_windows = spark_funcs.udf(
        ...
        spark_types.ArrayType(
            spark_types.MapType(spark_types.StringType(), spark_types.DateType())
        ),
    )
    ...
    adjusted_date_windows = collect_date_windows.select(
        "*",
        spark_funcs.explode(
            udf_adjust_date_windows(spark_funcs.col("array_coverage_windows"))
...
class SPD(QualityMeasure):  # pragma: no cover
    ...
    def _calc_measure(
        ...
        **kwargs
            ...
            .withColumn(
                ...
                spark_funcs.when(
                    ...
                    spark_funcs.regexp_extract(
                        spark_funcs.col("code_system"), r"\d+", 0
                    ),
                ),
            )
        )
        ...
        med_rx_combo_df = med_event_df.union(rx_event_df).distinct()
        ...
        rate_one_output_df = (
            ...
            .select(
                ...
                spark_funcs.when(
                    rate_one_numer_df.member_id.isNotNull(), spark_funcs.lit(1)
                )
                .otherwise(spark_funcs.lit(0))
                .alias("comp_quality_numerator"),
                spark_funcs.when(
                    rate_one_denom_df.member_id.isNotNull(), spark_funcs.lit(1)
                )
                .otherwise(spark_funcs.lit(0))
        ...
        rate_two_output_df = (
            ...
            .select(
                ...
                spark_funcs.when(spark_funcs.col("pdc") > 0.8, spark_funcs.lit(1))
                .otherwise(spark_funcs.lit(0))
                .alias("comp_quality_numerator"),
                spark_funcs.when(
                    rate_one_numer_df.member_id.isNotNull(), spark_funcs.lit(1)
        ...
        return rate_one_output_df.union(rate_two_output_df).orderBy("member_id")
