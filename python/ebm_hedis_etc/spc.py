...
def _identify_events(
    ...
    restricted_claims_df = claims_df.join(
        members_no_gaps_df, "member_id", how="inner"
    ).where(
        (
...
def _measure_exclusions(
        ...
        .join(
            spark_funcs.broadcast(
                rx_reference_df.where(
                    spark_funcs.col("medication_list")
    ...
    return (
        pregnancy_member_excl_df.union(ivf_member_excl_df)
    ...
    def _calc_measure(
        ...
        **kwargs
        ...
        eligible_males_membership_df = (
            ...
            .join(
                dfs_input["member"].select("member_id", "dob", "gender"),
                "member_id",
                how="left_outer",
            )
            .where(
        ...
        rate_one_output_df = (
            ...
            .select(
                ...
                .otherwise(spark_funcs.lit("SPC: Rate One (Females 40-75)"))
        ...
        rate_two_output_df = (
            ...
            .select("member_id", "gender")
            .join(
                rate_one_numer_df.withColumnRenamed("member_id", "denom_member_id"),
                dfs_input["member"].member_id == spark_funcs.col("denom_member_id"),
                how="left_outer",
            )
            .join(
                rate_two_numer_df.withColumnRenamed("member_id", "numer_member_id"),
            ...
            .select(
                ...
                .otherwise(spark_funcs.lit("SPC: Rate Two (Males 21-75)"))
                .alias("comp_quality_short"),
                spark_funcs.when(rate_two_numer_df.pdc >= 0.8, spark_funcs.lit(1))
                .otherwise(spark_funcs.lit(0))
                .alias("comp_quality_numerator"),
                spark_funcs.when(
                    spark_funcs.col("denom_member_id").isNotNull(), spark_funcs.lit(1)
                )
                .otherwise(spark_funcs.lit(0))
                .alias("comp_quality_denominator"),
                spark_funcs.lit(None).cast("string").alias("comp_quality_date_last"),
        ...
        rate_one_no_strat_df = rate_one_output_df.withColumn(
            "comp_quality_short", spark_funcs.lit("SPC: Rate One (Total)")
        )

        rate_two_no_strat_df = rate_two_output_df.withColumn(
            "comp_quality_short", spark_funcs.lit("SPC: Rate Two (Total)")
        )

        return (
            rate_one_output_df.union(rate_two_output_df)
            .union(rate_one_no_strat_df)
            .union(rate_two_no_strat_df)
            .orderBy("member_id")
        )
