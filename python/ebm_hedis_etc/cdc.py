...
def _rx_dispensed_event(
    rx_claims: DataFrame, rx_reference: DataFrame, performance_yearstart: datetime.date
) -> DataFrame:  # pragma: no cover
...
def _identify_med_event(
    ...
    restricted_med_claims = (
        ...
        .distinct()
        .withColumn("diag", spark_funcs.explode(spark_funcs.col("diag_explode")))
        .drop("diag_explode")
        .distinct()
        .where(
            spark_funcs.col("fromdate").between(
    ...
    acute_diags_explode_df = acute_inpatient_df.select(
        "member_id", "fromdate", restricted_med_claims.icdversion, "diag"
    )

    acute_diabetes_encounter_members = (
        acute_diags_explode_df.join(
            spark_funcs.broadcast(
                reference_df.where(spark_funcs.col("value_set_name").isin("Diabetes"))
            ),
            [
    ...
    non_acute_encounters_df = (
        restricted_med_claims.join(
            spark_funcs.broadcast(
                reference_df.where(
                    spark_funcs.col("code_system").isin("UBREV")
                    & spark_funcs.col("value_set_name").isin(
                        "Outpatient", "ED", "Nonacute Inpatient"
                    )
                )
            ),
            spark_funcs.col("revcode") == spark_funcs.col("code"),
    ...
    non_acute_diabetes_encounter_members = (
        non_acute_diags_explode_df.join(
            spark_funcs.broadcast(
                reference_df.where(spark_funcs.col("value_set_name").isin("Diabetes"))
            ),
            [
        ...
        .groupBy("member_id")
        .agg(spark_funcs.countDistinct("fromdate").alias("distinct_fromdate_count"))
        .where(spark_funcs.col("distinct_fromdate_count") >= 2)
        .select("member_id")
        .distinct()
    )
...
def identify_a1c_tests(
    ...
    return (
        claims_df.join(eligible_members, "member_id", how="inner")
        .where(
            spark_funcs.col("fromdate").between(
...
def identify_nephropathy(
    ...
    restricted_claims_df = claims_df.join(
        eligible_members, "member_id", how="inner"
    ).where(
        spark_funcs.col("fromdate").between(
            spark_funcs.lit(performance_yearstart),
            spark_funcs.lit(datetime.date(performance_yearstart.year, 12, 31)),
    ...
    ace_inhibitor_df = (
        rx_claims_df.join(eligible_members, "member_id", how="inner")
        .where(
            spark_funcs.col("fromdate").between(
                spark_funcs.lit(performance_yearstart),
                spark_funcs.lit(datetime.date(performance_yearstart.year, 12, 31)),
    ...
    nephrologist_df = (
        restricted_claims_df.where(spark_funcs.col("specialty") == "39")
        .select("member_id")
        .distinct()
    )
    ...
    return (
        ...
        .union(diag_df)
        .union(proc_df)
        .union(ace_inhibitor_df)
        .union(nephrologist_df)
        .distinct()
    )
...
def identify_eye_exam(
    ...
    screening_df = (
        restricted_claims_df.join(
            spark_funcs.broadcast(
                reference_df.where(
                    spark_funcs.col("value_set_name").isin("Diabetic Retinal Screening")
                )
            ),
            spark_funcs.col("hcpcs") == spark_funcs.col("code"),
            how="inner",
        )
        .where(spark_funcs.col("specialty").isin("18", "41"))
        .select("member_id")
        .distinct()
    )

    screening_diag_prior_df = (
        restricted_claims_prior_df.join(
            spark_funcs.broadcast(
                reference_df.where(
                    spark_funcs.col("value_set_name").isin("Diabetic Retinal Screening")
                )
        ...
        .select(
            ...
            spark_funcs.explode(
                spark_funcs.array(
                    [
                        spark_funcs.col(col)
                        for col in restricted_claims_prior_df.columns
                        if col.find("icddiag") > -1
                    ]
                )
            ).alias("diag"),
    ...
    bilateral_df = (
        ...
        .where(
            spark_funcs.col("modifier").isin("50")
            | spark_funcs.col("modifier2").isin("50")
        )
        .select("member_id")
        .distinct()
    )
    ...
    two_unilateral_df = (
        two_unilateral_pre_df.join(
            ...
            [
                ...
                == spark_funcs.col("join_value_set_name"),
                spark_funcs.datediff(
                    spark_funcs.col("join_fromdate"), spark_funcs.col("fromdate")
                )
                >= 14,
            ],
            how="inner",
    ...
    left_and_right_df = (
        ...
        .groupBy("member_id")
        .agg(spark_funcs.countDistinct("value_set_name").alias("left_or_right_count"))
        .where(spark_funcs.col("left_or_right_count") >= 2)
        .select("member_id")
        .distinct()
    )

    return (
        screening_df.union(screening_diag_prior_df)
        .union(screening_prof_df)
        .union(negative_result_df)
        .union(bilateral_df)
        .union(two_unilateral_df)
        .union(left_and_right_df)
        .distinct()
    )


def create_output_table(
    members: DataFrame, denominator: DataFrame, numerator: DataFrame, measure_name: str
) -> DataFrame:  # pragma: no cover
    """Prep numerator and denominator information for appropriate output format"""
    return (
        members.join(denominator, "member_id", how="inner")
        .join(numerator, members.member_id == numerator.member_id, how="left_outer")
        .select(
            members.member_id,
            spark_funcs.lit(measure_name).alias("comp_quality_short"),
            spark_funcs.when(numerator.member_id.isNotNull(), spark_funcs.lit(1))
...
class CDC(QualityMeasure):  # pragma: no cover
    ...
    def _calc_measure(
        ...
        pharmacy_eligible_members_df = _rx_dispensed_event(
            dfs_input["rx_claims"], dfs_input["ndc"], performance_yearstart
        )

        medical_eligible_members_df = _identify_med_event(
            dfs_input["claims"], reference_df, performance_yearstart
        )
        ...
        eligible_members_df = (
            ...
            .join(eligible_event_diag_members_df, "member_id", how="inner")
            .where(
        ...
        eligible_members_no_gaps_df = (
            eligible_members_df.join(
                _exclude_elig_gaps(eligible_members_df, 1, 45).withColumnRenamed(
        ...
        eye_exam_output_df = create_output_table(
            dfs_input["member"], eligible_members_no_gaps_df, eye_exam_df, "CDC: EYE"
        )

        nephropathy_output_df = create_output_table(
            dfs_input["member"], eligible_members_no_gaps_df, nephropathy_df, "CDC: NEP"
        )

        return (
            hba1c_output_df.union(eye_exam_output_df)
            .union(nephropathy_output_df)
            .orderBy("member_id")
        )
