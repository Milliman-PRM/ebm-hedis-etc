...
def test_find_elig_gaps(spark_app):
    test_input = spark_app.session.read.csv(
        str(_MOCK_DATA / "elig_gaps_input.csv"), header=True
    )
    expected_output = spark_app.session.read.csv(
        str(_MOCK_DATA / "elig_gaps_output.csv"), header=True
    )
    function_output = find_elig_gaps(
        test_input, spark_funcs.lit("2017-01-01"), spark_funcs.lit("2018-12-31")
    )
...
def test_flag_gap_exclusions(spark_app):
    ...
    test_input = test_input.select(
        [
            spark_funcs.col(col).alias(col.replace("expected_", ""))
            for col in test_input.columns
        ]
    )
    expected_output = spark_app.session.read.csv(
        str(_MOCK_DATA / "gap_exclusions_output.csv"), header=True
    )
    function_output = flag_gap_exclusions(test_input, 45, 1)
