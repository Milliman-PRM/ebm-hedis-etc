...
def compare_actual_expected(
    ...
    assert not unexpected, "Row(s) '{}' found in actual but not expected".format(unexpected)
...
def test_pcp_followup(mock_dataframe, expected_dataframe):
    ...
    for cutoff in CUTOFFS:
        ...
        result_expected = expected_dataframe.select(
            'member_id',
            spark_funcs.lit('PCP Followup: {}-Day'.format(cutoff)).alias('comp_quality_short'),
            spark_funcs.col(
                '{}day_comp_quality_numerator'.format(cutoff)
