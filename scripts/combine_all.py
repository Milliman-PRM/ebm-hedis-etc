...
def create_quality_measures_output(
    sparkapp: SparkApp, quality_measure_dir: IndyPyPath
) -> DataFrame:
    ...
    for measure_path in quality_measure_dir.collect_files_regex("results"):
        ...
        if not quality_measures_df:
            ...
            quality_measures_df = quality_measures_df.union(
                measure_df.select(quality_measures_df.columns)
            )

    return (
        quality_measures_df.where(spark_funcs.col("comp_quality_denominator") != 0)
        .withColumn(
...
def main() -> int:
    ...
    quality_measures_df = create_quality_measures_output(sparkapp, PRM_META[150, "out"])

    ref_quality_measures_df = sparkapp.load_df(
        PATH_REF / "hedis_ref_quality_measures.parquet"
    ).withColumn(
        "comp_quality_target_value",
        spark_funcs.col("comp_quality_target_value")
        .cast("double")
        .alias("comp_quality_target_value", metadata={"label": "Target"}),
    )

    sparkapp.save_df(
        quality_measures_df, PRM_META[150, "out"] / "quality_measures.parquet"
    )
    write_sas_data(
        quality_measures_df, PRM_META[150, "out"] / "quality_measures.sas7bdat"
    )

    sparkapp.save_df(
        ref_quality_measures_df, PRM_META[150, "out"] / "ref_quality_measures.parquet"
    )
    write_sas_data(
        ref_quality_measures_df, PRM_META[150, "out"] / "ref_quality_measures.sas7bdat"
    )
